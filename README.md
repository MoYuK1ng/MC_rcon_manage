...
- **[API Documentation](#api-endpoints)** - Available endpoints and usage
...
- 🔐 **安全认证** - 基于 Django 的用户认证和组权限管�?
- 🎮 **多服务器管理** - 从一个仪表板管理多个 Minecraft 服务�?
- 👥 **实时玩家监控** - 自动刷新玩家列表�?0 秒轮询）
- �?**白名单管�?* - 通过 RCON 将玩家添加到服务器白名单
- 🔒 **加密凭证** - 使用 Fernet 对称加密保护 RCON 密码
- 🌍 **国际�?* - 完整支持英语和简体中�?
- 📱 **响应式设�?* - Bootstrap 5 UI 在所有设备上无缝工作
- �?**现代技术栈** - HTMX 实现无需页面重载的动态更�?
...
- **[翻译指南](locale/README.md)** - 如何添加或更新翻�?
- **[API 文档](#api-endpoints)** - 可用端点和使用方�?
...
- **访问控制**：基于组的权限和 `@user_has_server_access` 装饰�?
- **CSRF 保护**：为 HTMX 请求配置 Django CSRF 令牌
- **安全默认�?*：生产设置包�?HSTS、安�?cookie �?
...
| `/server/<id>/players/` | GET | 玩家列表�?HTMX 端点 |
| `/server/<id>/whitelist/` | POST | 将玩家添加到白名�?|
| `/i18n/setlang/` | POST | 语言切换�?|
