...
MC RCON Manager provides easy-to-use scripts for backing up and restoring your database.
...
# You will be prompted to enter a backup directory
# Press Enter to use the current directory, or specify a custom path
```

**What happens:**
- Creates a timestamped backup file: `db_backup_YYYYMMDD_HHMMSS.sqlite3`
- Displays backup information (filename, location, size)
- Shows instructions for restoring the backup
...
📋 Backup Information:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Filename: db_backup_20250130_143022.sqlite3
  Location: /opt/mc_rcon/db_backup_20250130_143022.sqlite3
  Size:     2.5M
  Date:     2025-01-30 14:30:22
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💡 To restore this backup, select '9) Restore Data' from main menu
```
...
# Enter the full path to your backup file when prompted
```

**What happens:**
1. Shows backup file information
2. Asks for confirmation (type `yes` to continue)
3. Creates a backup of your current database (safety measure)
4. Stops the MC RCON service
5. Restores the backup file
6. Starts the MC RCON service
7. Verifies service is running

**Important Notes:**
- Always create a backup before major changes
- The restore function automatically backs up your current database before restoring
- Service is automatically stopped and restarted during restore
- Keep multiple backups in different locations for safety

#### Automated Backups (Recommended)

You can create a script for automated backups:
...
crontab -e
# Add this line:
0 2 * * * /opt/mc_rcon/auto_backup.sh >> /var/log/mc_rcon_backup.log 2>&1
```

#### Backup Best Practices

1. **Regular Backups**: Schedule daily automated backups
2. **Multiple Locations**: Store backups in different locations (local + remote)
...
5. **Before Key Rotation**: Backup before rotating encryption keys
6. **Retention Policy**: Keep at least 7 daily backups and 4 weekly backups
...
MC RCON Manager 提供了易于使用的脚本来备份和恢复数据库。
...
# 选择选项 8) 备份数据
# 系统会提示您输入备份目录
# 按 Enter 使用当前目录，或指定自定义路径
```

**执行过程：**
- 创建带时间戳的备份文件：`db_backup_YYYYMMDD_HHMMSS.sqlite3`
- 显示备份信息（文件名、位置、大小）
- 显示恢复备份的说明
...
📋 备份信息：
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  文件名: db_backup_20250130_143022.sqlite3
  位置:   /opt/mc_rcon/db_backup_20250130_143022.sqlite3
  大小:   2.5M
  日期:   2025-01-30 14:30:22
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💡 要恢复此备份，请在主菜单选择 '9) 恢复数据'
```
...
1. **定期备份**：安排每日自动备份
2. **多个位置**：将备份存储在不同位置（本地 + 远程）
3. **测试恢复**：定期测试备份恢复过程
4. **更新前备份**：更新应用程序前始终备份
5. **密钥轮换前备份**：轮换加密密钥前备份
6. **保留策略**：至少保留 7 个每日备份和 4 个每周备份
